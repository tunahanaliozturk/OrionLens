# Contributing to OrionLens

Thanks for taking the time to look at this. Ambient correlation context for .NET: a correlation id and baggage that flow through an async call chain and propagate across HTTP boundaries via an ASP.NET Core middleware and an HttpClient handler. The project is small and the bar for contributions is "does it make the package clearer, faster, or safer without expanding the public surface needlessly."
...
```bash
git clone https://github.com/tunahanaliozturk/OrionLens
cd OrionLens
dotnet restore
